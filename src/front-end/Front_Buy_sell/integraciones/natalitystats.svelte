...
async function getStats() {
    ...
    if (res.ok) {
      ...
      countries = json;
      console.log(`We have received ${evictions.length} evictions.`);
    
      console.log("Ok");
    } else {
      errorMsg="Error recuperando datos de evictions"
      okMsg = "";
...
async function loadChart(){
 ...
 for (let index = 0; index < evictions.length; index++) {
  const element = evictions[index];
  console.log("NOMBRE: "+element.name);
  
    xAxis.push(element.total);
    yAxis.push(element.other);
