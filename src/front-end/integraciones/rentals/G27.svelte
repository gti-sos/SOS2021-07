<script>
  ...
  async function getStats() {
    ...
    if (res.ok) {
      ...
      okMsg = "";
      console.log(`We have received ${Rentals_data.length} rentals.`);
    } else {
      console.log("Error");
  ...
  async function getIlliteracyData() {
    ...
    if (res.ok) {
      ...
      console.log(
        `We have received ${Social_data.length} budget.`
      );
  ...
  async function loadChart() {
    ...
    var result = jsonToMap(Social_data, "province", "invest_promotion");
    var result1 = jsonToMap(Rentals_data, "province", "rent");
...
data: arrayAux
    ...
    Highcharts.chart("container", {
      ...
      title: {
        text: "renta por provincias y inversion social",
      },
      xAxis: {
